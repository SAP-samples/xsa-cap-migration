...
sap.ui.controller("sap.hana.democontent.epm.job.view.app", {
	...
	loadJobsTable: function() {
		var oThis = this;
        var oTable = oThis.byId("manageJobsTable");
		$.ajax({
			type: "GET",
			url: "/v2/shine/JOBS_SCHEDULEDETAILS",
			async: true,
	...
	onButtonPress: function(oEvent, oController) {
		...
		if (oEvent.getSource() === this.byId("btnNew")) {
			...
			// 	item.appurl = "https://"+window.location.hostname+":"+window.location.port+"/v2/shine/jobCreate?jobname='"+ item.jobname + "'";
			// } else {
			// 	item.appurl = "https://"+window.location.hostname+"/v2/shine/jobCreate?jobname='"+ item.jobname + "'";
			// }
			var xsrf_token;
			$.ajax({
				type: "GET",
			...
			$.ajax({
				type: "POST",
				url: "/v2/shine/createjobschedule",
				headers: {
				...
				success: function(data, oController) {
					...
					var oJobName = data.d.createjobschedule;
				...
				error: function(jqXHR, textStatus, errorThrown) {
					...
					var message = jsonMessage.error.message.value;
	...
	loadJobActivitiesTable: function() {
		var oThis = this;
        var oTable = oThis.byId("jobActionsTable");
		$.ajax({
			type: "GET",
			url: "/v2/shine/JOBS_DATA",
			async: true,
			contentType: "application/json",
			dataType:'json',
			headers: {
	...
	deleteJob: function() {
		...
		if(jobId!="" && jobId!=null){
			...
			$.ajax({
				type: "GET",
				url: "/v2/shine/deletejobschedules?jobId='" + jobId + "'",
				// headers: {
				// 	'x-csrf-token': xsrf_token
				// },
				success: function(data) {
					sap.ui.commons.MessageBox.show(data.d.deletejobschedules,
						"SUCCESS",
