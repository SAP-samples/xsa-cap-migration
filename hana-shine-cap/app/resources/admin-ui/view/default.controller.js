...
sap.ui.controller("sap.hana.democontent.epm.admin.view.default", {
	...
	executeConfirm: function(bResult, oController) {
		...
		if (bResult) {
			if (oModel.getProperty("/cb1")) {
				var urls = [
					'/v2/shine/resetAddresses',
					'/v2/shine/resetEmployees',
					'/v2/shine/resetPartners',
					'/v2/shine/resetProducts',
					'/v2/shine/resetConstants',
					'/v2/shine/resetTexts',
					'/v2/shine/resetNotes',
					'/v2/shine/resetAttachments'
				];
				oController.reloadData(urls, oController, "cb1");

			} else if (oModel.getProperty("/cb2")) {
				var urls = [
					'/v2/shine/resetSOHeader',
					'/v2/shine/resetSOItem',
					'/v2/shine/resetPOHeader',
					'/v2/shine/resetPOItem'
				];
				...
				for (var i = 0; i < soRequired; i++) {
					jQuery.ajax({
						url: '/v2/shine/replicateSales()',
						method: 'GET',
						dataType: 'json',
						success: function(myTxt) {
							oModel.setProperty('/txtLog', myTxt.d.replicateSales + "\n" + oModel.getProperty('/txtLog'));
							soLoops++;
				...
				for (var j = 0; j < poRequired; j++) {
					jQuery.ajax({
						url: '/v2/shine/replicatePurchase()',
						method: 'GET',
						dataType: 'json',
						success: function(myTxt) {
							oModel.setProperty('/txtLog', myTxt.d.replicatePurchase + "\n" + oModel.getProperty('/txtLog'));
							soLoops++;
							oModel.setProperty('/percentValue',
								Math.round((soLoops + poLoops) / (soRequired + poRequired) * 100));
							oModel.setProperty('/displayValue',
			...
			// checkbox for time based data generator
				...
				oModel.setProperty('/percentValue', (soLoops + poLoops) * 1000);
					...
					var url = "/v2/shine/replicateTimebasedPO";
				...
				if (parseInt(oModel.getProperty('/SOVal')) !== 0) {
					...
					url = "/v2/shine/replicateTimebasedPO";
	...
	reloadData: function(urls, oController, msg) {
		...
		for (var i = 0; i < urls.length; i++) {
			jQuery.ajax({
				...
				dataType: 'json',
				success: function(myTxt) {
					var functionUrl = urls[i];
					let functionName = functionUrl.substring(10, functionUrl.length);
					oModel.setProperty('/txtLog', myTxt.d[functionName] + "\n" + oModel.getProperty('/txtLog'));
					resetCount++;
					oModel.setProperty('/percentValue',
	...
	triggerReplicateTimeBasedPO: function(oController, noRec, url, id) {
		...
		jQuery.ajax({
			...
			success: function(myTxt) {
				...
				oModel.setProperty('/txtLog', myTxt.d.replicateTimebasedPO + "\n" + oModel.getProperty('/txtLog'));
				...
				var totalNumberOfRecordsTxt = (myTxt.d.replicateTimebasedPO).split(":");
	...
	getTableSizes: function(oController) {
		var aUrl = '/v2/shine/getDBTableSize';
