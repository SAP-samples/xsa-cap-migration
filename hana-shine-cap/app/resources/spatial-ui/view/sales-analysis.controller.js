sap.ui.controller("shine.democontent.epm.spatial.view.sales-analysis", {
		...
		for (var j = 0; j < data.topBuyers.length; j++) {
			...
			custMarker.addEventListener("tap", function(evt) {
				...
				$.ajax({
					type: "GET",
					async: false,
					url: "/v2/shine/getBPSalesData?bpId=" + "'" + marker.getData().partnerID + "'",
					success: function(data) {
						var oModel = new sap.ui.model.json.JSONModel({});
						oModel.setData(data.d.getBPSalesData);
	...
	sendRequest: function(oController, payload) {
		...
		$.ajax({
			...
			url: "/v2/shine/getSalesAnalysis",
			success: function(data) {
				oController.addDataToMap(data.d.getSalesAnalysis, oController.getView(), oController);
			},
			error: function(err) {
