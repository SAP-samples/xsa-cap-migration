	...
	onAfterRendering: function() {
		...
		function onMarkerSelected(label, selectedBP, marker) {
			...
			$.ajax({
				type: "GET",
				async: false,
				url: "/v2/shine/getBPTransactionData?bpId=" + "'" + selectedBP.ID + "'" + "&lat=" +
					selectedBP.lat + "&long=" + selectedBP.long + "&userlat=" + lati + "&userlong=" + longi,
				success: function(data) {
					var sales;
					var oModel = new sap.ui.model.json.JSONModel({});
					if (!data.d.getBPTransactionData.salesTotal) {
						sales = 0;
					} else {
						sales = data.d.getBPTransactionData.salesTotal;
						// add sales chart
						oModel.setData(data.d.getBPTransactionData);
					}
					...
					(view.byId("bpHeader")).setNumberUnit(data.d.getBPTransactionData.salesYoY[0].currency);
					//for distance calculation

					//(view.byId("bpDistanceItem")).setValue(data.d.getBPTransactionData.distance / 1000);
				},
				error: function(err) {
		...
		$.ajax({
			type: "GET",
			async: false,
			url: "/v2/shine/getAllBusinessPartnersData()",
			success: function(data) {
				bps = data.d.getAllBusinessPartnersData.entry;
				for (var i = 0; i < data.d.getAllBusinessPartnersData.entry.length; i++) {
					labels[i] = data.d.getAllBusinessPartnersData.entry[i].Name;
					addresses[i] = data.d.getAllBusinessPartnersData.entry[i].Building + ' ' + data.d.getAllBusinessPartnersData.entry[i].Street + ', ' + data.d.getAllBusinessPartnersData.entry[i].Zip + ' ' + data.d.getAllBusinessPartnersData.entry[i].City + ', ' +
					data.d.getAllBusinessPartnersData.entry[i].Country;
				}
