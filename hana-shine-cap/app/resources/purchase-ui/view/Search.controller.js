sap.ui.controller("shine.democontent.epm.poworklist.view.Search", {
    ...
    loadFilter: function(oEvent) {
        ...
        if (gSearchParam.length >= 3) {
            var aUrl = "/v2/shine/getFilter?query=" + "'" + oEvent.getParameter("value") + "'";
            jQuery.ajax({
                url: aUrl,
                method: 'GET',
                dataType: 'json',
                success: jQuery.proxy(onLoadFilter1, this),
                error: onErrorCall
            });
        }
    },
...
function onLoadFilter1(myJSON) {
    var oSearchControl1 = this.byId("filterBy");
    var Jsonobj = JSON.parse(myJSON.d.getFilter);
    var aSuggestions1 = [];
    for (var i = 0; i < Jsonobj.length; i++) {
        aSuggestions1[i] = Jsonobj[i].terms + ' | ' + oBundle.getText("attribute") + ' ' + Jsonobj[i].attribute;
    }
    oSearchControl1.suggest(gSearchParam, aSuggestions1);
}
