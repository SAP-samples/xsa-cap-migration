sap.ui.controller("shine.democontent.epm.poworklist.view.Table", {
    ...
    onInit: function() {
        ...
        var sBusinessPartnersServiceUrl = "/v2/shine/";
        ...
        var sProductDetailsServiceUrl = "/v2/shine/";
    ...
    onTBPress: function(oEvent, oController) {
        ...
        if (oEvent.getSource() === this.byId("btnExcel")) {
            ...
            $.ajax({
                type: "GET",
                async: false,
                url: "/v2/shine/downloadExcel",
                success: function(data) {
                    sap.ui.core.util.File.save(data.d.downloadExcel, "POData", "csv", null, null, null);
                    sap.ui.commons.MessageBox.show("Export of excel succeeded", "SUCCESS", "Success");
                },
                error: function(jqXHR) {
                    sap.ui.commons.MessageBox.show("Export of excel failed", "ERROR", "Error");
                    return;
                }
            });
            return;
        ...
        if (oEvent.getSource() === this.byId("btnZip")) {
            ...
            $.ajax({
                type: "GET",
                async: false,
                url: "/v2/shine/downloadZip",
                success: function(data) {
                    sap.ui.core.util.File.save(data.d.downloadExcel, "POData", "zip", "application/zip", null, null);
                    sap.ui.commons.MessageBox.show("Export of zip succeeded", "SUCCESS", "Success");
                },
                error: function(jqXHR) {
                    sap.ui.commons.MessageBox.show("Export of zip failed", "ERROR", "Error");
                    return;
                }
            });
            return;
    ...
    submit: function(oEvent, oController) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var oPurchaseOrderId = data.d.poCreate.PURCHASEORDERID;
    ...
    deleteConfirm: function(bResult, oController, poId) {
        if (bResult) {
            var payload = { "PURCHASEORDERID": poId };
            var xsrf_token;
            $.ajax({
            	type: "GET",
            	async: false,
    ...
    approvalConfirm: function(bResult, oController, poId, action) {
        if (bResult) {
            var payload = { "PURCHASEORDERID": poId, "Action": action };
            	var xsrf_token;
            $.ajax({
            	type: "GET",
            	async: false,
    ...
    onErrorCall: function(jqXHR, textStatus, errorThrown) {
        if (jqXHR.status == '500') {
            sap.ui.commons.MessageBox.show(jqXHR.responseJSON.error.message.value,
                "ERROR",
                oBundle.getText("error_action"));
            return;
        } else {
            sap.ui.commons.MessageBox.show(decodeURI(jqXHR.responseJSON.error.message.value),
                "ERROR",
                oBundle.getText("error_action"));
            return;
        }
    ...
    onAfterRendering: function(oEvent) {
        ...
        var oModel = new sap.ui.model.odata.ODataModel("/v2/shine/", true);
