sap.ui.controller("shine.democontent.epm.poworklist.view.Detail", {
    ...
    setGroupBy: function(oEvent, oController) {
        ...
        var aUrl = "/v2/shine/getTotalOrders?groupby=" + "'" + groupBy + "'" + "&currency='USD'";
        jQuery.ajax({
            url: aUrl,
            method: 'GET',
            dataType: 'json',
            success: onLoadTotals1,
            error: onErrorCall
        });
...
function onLoadTotals1(myJSON) {
    var Jsonobj = myJSON.d.getTotalOrders;
    var data = [];
    for (var i = 0; i < Jsonobj.entries.length; i++) {
        data[i] = {
            label: Jsonobj.entries[i].name,
            data: Jsonobj.entries[i].value
        };
    }
    oPieModel.setData({
        modelData: data
